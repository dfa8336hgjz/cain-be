...
async def create_notebook(title: str, user_id: str = Depends(validate_token), controller: AppController = Depends(get_app_controller)):
    ...
    await notebook_controller.create_notebook_by_user_id(user_id, title)
    return JSONResponse(content={'message': f'Notebook {title} created successfully'}, status_code=200)
